...
class MLAAttention(nn.Module):
    ...
    def _forward_decode(
        ...
        if envs.ATOM_USE_TRITON_MLA:
            from atom.model_ops.triton_decode_attention import decode_attention_fwd
            k_buffer = kv_c_and_k_pe_cache.unsqueeze(2)
            v_buffer = k_buffer[..., : self.kv_lora_rank]
            page_size = k_buffer.shape[1]

            # Cast fp8 Q to bf16 (ROCm Triton can't do element-wise on fp8)
            q_for_triton = q.to(torch.bfloat16) if q.dtype.is_floating_point and q.element_size() == 1 else q
